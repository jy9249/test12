...
public class jv08_01_String메서드 {
    ...
    public static void main(String[] args) {
        ...
        int length = prov.length(); // 배열식으로 나타냄
        System.out.println(prov.length());
        // 문자열 추출 : substring()
        // (2번째부터 4번째 자리까지 추출: bar)
        String bar = prov.substring(2, 5); // 5까지해줘야함
        ...
        // prov 문자열에 Dog가 들어 있으면 Cat으로 바꾸어 출력 아니면 prov 출력
        
        if (contains = true) {
            prov.replace("Dog", "Cat");
            System.out.println(s07);
        } else {
        ...
        String[] arr = prov.split(" "); // String[] arr의 배열로 돌려준다.
        ...
        if(s09.equals(s10)){ // == 보다는 equals 로 기억하는 것이 좋다.
